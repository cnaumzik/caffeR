prepareHdf5 <- function(caffedir = "~/Documents/caffe" ,
  ...
  h5createDataset(file_name , "data", c(n,3,resize_width,resize_height), storage.mode = "double", showWarnings = FALSE,level=9 ,chunk = c(batch_size,3,resize_width,resize_height))
  h5createDataset(file_name , "label", c(n,1), storage.mode = "double", showWarnings = FALSE,level = 9, chunk =c(batch_size,1))
  i<-1
  chunks <-0
  for(k in 1:n){
    print(k)
    image_path <- paste0("~/Promotion/Research/Code/Data/Catsvsdogs/main/",image_ids[k],".jpg")
    print(image_path)
    ...
    if(i==batch_size+1){
      ...
      h5write(data_batch, file = file_name, name ="data", start = c(lower,1,1,1),count=c(batch_size,3,resize_width,resize_height))
      #h5write(label_batch, file = file_name, name ="label", index = list(lower:upper,1))
      h5write(label_batch, file = file_name, name ="label", start = c(lower,1),count=c(batch_size,1))
      chunks <- chunks +1
    }
  }
