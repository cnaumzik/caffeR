...
WriteMakeMean <- function(caffedir="~/Documents/caffe" , name = "MyModel"){
  ...
  source_path <- paste0(caffedir,"/examples/imagenet/make_imagenet_mean.sh")
  ...
  new_MakeMean[9] <- paste0("$TOOLS/compute_image_mean $EXAMPLE/",name,"_train_lmdb \\")

  new_MakeMean[10]<- paste0("$DATA/",name,"_train_mean.binaryproto")

  new_MakeMean[12] <- paste0("$TOOLS/compute_image_mean $EXAMPLE/",name,"_val_lmdb \\")
