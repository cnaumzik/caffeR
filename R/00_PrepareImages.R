PrepareImages <- funtion(caffedir = "~/Documents/caffe" ,name ="MyModel" , imagedir = "~/Documents/data" , labels , image_ids, suffix = NULL, preprocessing = FALSE , share_val = 0.1 , seed_no = 12345678 ){
  ...
  for (k in 1:n) {
    ...
    if (which(names[k] %in% validation_images)) {
      ...
      target_path <- paste0(caffedir,"/data",name,"/val",image_ids[k],".jpg")
    ...
    #ImagePreprocessing(file_path , target_path , preprocessing , Resize_height,Resize_width)
