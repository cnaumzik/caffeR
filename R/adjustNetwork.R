#' @export
adjustNetwork <- function(caffedir = "~/Documents/caffe" , name = "My_model", network_name = "bvlc_reference_caffenet" , no_new_layers = 1 , freeze_all = TRUE , lr = NULL, num_output = 2 ){
...
if(length(layer_indx)-1 < no_new_layers){
  stop(paste0("This network only has ", length(layer_indx)-1," layers. Choose a different network or give a lower number of layers to be changed."))
...
for (k in 1 :(length(layer_indx)-1)) {
  ...
  current_layer <- adjustLayer(current_layer, caffedir , name , vision_lr[k] , new_layer[k] , new_bottom[k] )
...
current_layer <- file[layer_indx[k]:(layer_indx[k+1]-1)]
current_layer[grep("num_output",current_layer)] <- paste0("num_output: ",num_output)
file[layer_indx[k]:(layer_indx[k+1]-1)] <- current_layer
...
identifyNewLayers <- function (file = NULL , no_new_layers = 1){
  ...
  while(i > 0) {
    ...
    if(length(grep("Data" , layer[indx])) > 0 || length(grep("loss" , layer[indx] , ignore.case = TRUE)) > 0 || length(grep("accuracy" , layer[indx], ignore.case = TRUE)) > 0) {
  ...
  if(no_new_layers > 0) {
    ...
    while (k <= (n-1)){
      ...
      bottom <- gsub("\"", "", gsub("[A-z]*:\\s" , "" , trimws(layer[grep("bottom" , layer)])))
      top <- gsub("\"", "", gsub("[A-z]*:\\s" , "" , trimws(layer[grep("top" , layer)])))
...
adjustLayer <- function(layer, caffedir = "~/Documents/caffe", name = "My_model", vision_lr = 1 , new_layer , new_bottom){
  ...
  } else if (length(grep("loss" , layer[indx] , ignore.case = TRUE)) > 0 || length(grep("accuracy" , layer[indx] , ignore.case = TRUE)) > 0) {

    layer <- adjustLossLayer(layer, name , new_bottom )

  } else if (length(grep("Convolution" , layer[indx])) > 0 || length(grep("Pooling" , layer[indx])) > 0 || length(grep("LRN" , layer[indx])) > 0 || length(grep("InnerProduct" , layer[indx])) > 0 ) {
...
adjustDataLayer <- function(layer , caffedir = "~/Documents/caffe" , name = "My_model" , batch_size_train = 64 , batch_size_val = 32){
  ...
  if(length(grep("TRAIN",layer)) > 0){
    ...
    layer[grep("mean_file",layer)]<- paste0("mean_file: \"" , caffedir , "/data/" , name , "/" , name , "_train_mean.binaryproto\"")

    layer[grep("source",layer)] <- paste0("source: \"" , caffedir , "/examples/", name , "/" , name , "_train_lmdb\"")

    layer[grep("batch_size",layer)] <- paste0("batch_size: ", batch_size_train)

  } else {

    layer[grep("mean_file",layer)]<- paste0("mean_file: \"" , caffedir , "/data/" , name , "/" , name , "_val_mean.binaryproto\"")

    layer[grep("source",layer)] <- paste0("source: \"" , caffedir , "/examples/", name , "/" , name , "_val_lmdb\"")

    layer[grep("batch_size",layer)] <- paste0("batch_size: ", batch_size_val)
...
adjustLossLayer <- function(layer , name = "My_model" , new_bottom = TRUE , loss ="EuclideanLoss") {
  ...
  if (new_bottom) {
    ...
    layer[bottom_indx[change_indx]] <- changeLayerName(layer[bottom_indx[change_indx]])
...
adjustVisionLayer <- function(layer , name = "My_model" , vision_lr = 1 , new_layer = FALSE , new_bottom = FALSE){
  ...
  if (new_layer) {
    ...
    layer[grep("name" , layer)] <- changeLayerName(layer[grep("name" , layer)] , name)
    layer[grep("top" , layer)] <- changeLayerName(layer[grep("top" , layer)] , name)

    if(new_bottom) {

      layer[grep("bottom" , layer)] <- changeLayerName(layer[grep("bottom" , layer)] , name)
  ...
  layer[grep("lr_mult" , layer)] <- gsub("[0-9]*$",vision_lr,layer[grep("lr_mult" , layer)])

  return(layer)
}
#========================================================================================================================================================
adjustActivationLayer <- function(layer , name = "My_model" , new_layer = FALSE , new_bottom = FALSE){
...
changeLayerName <- function(currentName , name = "My_model"){
  ...
  extracted_name <- gsub("\"", "", gsub("[A-z]*:\\s" , "" , trimws(currentName)))

  new_name <- gsub(extracted_name , paste0(extracted_name , "-" , name) , currentName)
